  ...
  public partial class Converter : Page
  {
    ...
    protected void CalculateButton_Click(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        {
          TextBox2.Text = Mathos.Converter.Converter.From(a, Convert.ToDouble(TextBox1.Text)).To(b).ToString();
        }
        else
        {
          // need to make this work for all bases (now: only for base 10 and higher)
          TextBox2.Text = Mathos.Converter.Converter.From((Base) a, TextBox1.Text).To((Base) b);
        }
    ...
    protected void SetValues(int type)
    {
      ...
      {
        var item = new ListItem(itemvalues[i].ToString());
